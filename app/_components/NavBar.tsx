...
// ];
const Icons: ILinks[] = [
  { label: "lucide:user", href: "/login" },
  { label: "fluent:cart-24-regular", href: "/login" },
];
const authIcons: ILinks[] = [
  { label: "fluent:cart-24-regular", href: "/" },
  { label: "ion:notifications-outline", href: "/" },
];
...
const NavBar = () => {
  ...
  return (
    <nav className="flex justify-between items-center bg-white px-5 md:px-7 lg:px-20 w-full h-16 md:h-24 shadow-md">
      <SmartLogo />
      <ul className="xl:flex flex-1 justify-start items-center space-x-8 xl:space-x-9 hidden ml-14">
      ...
      {isLoggedIn === null ? (
        ...
        <section className="flex items-center space-x-2 md:space-x-[22px]">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <div className="flex items-center space-x-2 font-semibold text-lg md:text-xl">
                  ...
                  <Image
                    className="ml-2 w-4 h-4 xl:w-[10px] xl:h-[10px]"
                    alt="smart"
          ...
          <div className="xl:flex space-x-3 hidden">
            {(isAuthenticated ? authIcons : Icons).map((icon, index) => (
              <Link
                href={session ? `${icon.href}` : `/login`}
                key={index}
                className="border-darkGray text-darkGray hover:border-primaryRed hover:text-primaryRed p-[9px] border rounded-full w-11 h-11 duration-300 cursor-pointer"
              >
                {/* <Image
                  src={icon.label}
                  alt="smart"
              ...
              <div className="flex space-x-5 mx-auto w-fit">
                {(isAuthenticated ? authIcons : Icons).map((icon, index) => (
                  <Link
                    href={icon.href}
                    key={index}
                    className="border-darkGray hover:border-primaryRed p-[9px] border rounded-full w-11 h-11 hover:text-primaryRed duration-300 cursor-pointer"
                  >
